...
class Controller:
    ...
    def start_default_state(self):
        ...
        # gradually move to default pos state (for all joints)
    ...
    def keep_default_state(self):
        """
        Keep the robot in this default state
        """
    ...
    def run(self):
        ...
        quat = np.array(self.low_state.imu_state.quaternion)    # global
        print("\n\n quaternion: ", quat)
        ang_vel = np.array(self.low_state.imu_state.gyroscope)  # local
        ...
        self.obs[:3] = ang_vel                                                  # pelvis angular velocity in local/pelvis frame
        self.obs[3:6] = gravity_orientation                                     # gravity orientation in pelvis frame
        print("cmd: ", self.cmd[0])
        print("gravity_orientation: ", gravity_orientation)
        accel = self.low_state.imu_state.accelerometer
        print("accel: ", accel)
        ...
        if (self.remote_controller.button[KeyMap.A] == 1):
            target_upper_pos = mode_salute_left_arm()

        if (self.remote_controller.button[KeyMap.B] == 1):
            target_upper_pos = mode_swing_right_arm()
